...
public class LightingEngine {
	...
	static {
		for (int i = 0; i < 6; ++i) {
			final Vec3i offset = IMixinEnumFacing.getValues()[i].getDirectionVec();
			...
			for (this.curQueue = this.queuedDarkenings[curLight]; this.nextItem(); ) {
				...
				if (this.calcNewLightFromCur(luminosity, opacity) < curLight) {
					...
					for (int i = 0; i < 6; ++i) {
						...
						if (nChunk == null) {
							LightingHooks.flagSecBoundaryForUpdate(this.curChunk, this.curPos, this.lightType, IMixinEnumFacing.getValues()[i], LightingHooks.EnumBoundaryFacing.OUT);
							continue;
						}
	...
	private int calcNewLightFromCur(final int luminosity, final int opacity) {
		...
		for (int i = 0; i < 6; ++i) {
			if (this.neighborsChunk[i] == null) {
				LightingHooks.flagSecBoundaryForUpdate(this.curChunk, this.curPos, this.lightType, IMixinEnumFacing.getValues()[i], LightingHooks.EnumBoundaryFacing.IN);
				continue;
			}
	...
	private int posToCachedLight(final MutableBlockPos pos, final Chunk chunk) {
		return ((IChunk) chunk).getCachedLightFor(this.lightType, pos);
	}
